...
;;
;; The 'Insert Pair Edit' package defines a command `insert-pair-edit'
;; that can be configured as a more feature rich alternative to the
;; standard 'M-(' Emacs keybinding `insert-parentheses'.
;;
...
;;
;; Once inserted, `insert-pair-edit' will run the command
...
package."
  ...
  :link '(emacs-commentary-link "insert-pair-edit.el")
   ...
   '(("("  "("  ")")
     ...
     ("'"  "'"  "'"
      (
       ...
       :escapes (("'"  "\\'")  ("\\" "\\\\"))))
     ("`"  "`"  "`")))
  "Opening / Closing PAIRS inserted by `insert-pair-edit'.
...
- MNEMONIC is an input entered into the minibuffer by the user after
  ...
  are to be inserted into the buffer around the lexical unit at POINT.
...
- :auto-insert - A boolean which, if non-nil, will cause the
  ...
  entering `ipe-edit-mode'.  Optional.  Default: nil
...
(defcustom ipe-mode-pairs '()
  "Modal PAIRS inserted by `insert-pair-edit' (ipe).
...
If nil, the `insert-pair-edit' prompt that reads a MNEMONIC will
...
If nil, `insert-pair-edit-update' / `insert-pair-edit-delete' and
`insert-pair-edit-replace' will first search backward, then forward
for a PAIR.

If non-nil, `insert-pair-edit-update' / `insert-pair-edit-delete' and
`insert-pair-edit-replace' will first search forward, then backward
for a matching PAIR."
  :group 'ipe-advanced
  :link  '(function-link insert-pair-edit-update)
...
If non-nil, a numeric ARG to `insert-pair-edit' will move the CLOSE
string forward.

If nil, a numeric ARG to `insert-pair-edit' will move the OPEN string
backward."
  :group 'ipe-advanced
  :link  '(function-link insert-pair-edit)
...
`insert-pair-edit' while editing in Insert Pair Edit minor
mode.
...
`insert-pair-edit' while editing in Insert Pair Edit `ipe-edit-mode'
minor mode.
...
CLOSE overlays by `insert-pair-edit' while editing in Insert Pair
...
for which the annotation is to be returned."
  ...
  ;; `insert-pair-edit' is being run, but the annotate function
